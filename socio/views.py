...
def favorite(request, id):
    ...
    if post.favorites.filter(id=request.user.id).exists():
        ...
        messages.success(
            request, f'Post Saved! You can check it out in your Bookmarks.')
...
def filter_list(request):
    f = PostFilter(
        request.GET, queryset=Post.objects.all().order_by('-date_posted'))
    ...
    if request.method == 'POST':
        ...
        if doc.is_valid():
            ...
            f = os.path.dirname(os.path.dirname(
                os.path.abspath(str(imgname)))).replace('\\', '/')
...
def trending(request):
    post = Post.objects.annotate(like_count=Count(
        'likes')).order_by('-like_count', '-date_posted')
    context = {
        'title': 'Trending',
        'posts': post,
    }
...
def dashboard(request):
    ...
    logged_in_user_posts = Post.objects.filter(
        author=logged_in_user).order_by('-date_posted')
